...
def get_data_loader(dataset, norm_stat=None, train_bs=64, test_bs=64,
                    validation=False, validation_fraction=0.1, root_dir='data/',
                    test_only=False, train_only=False, augment=True,
                    num_train_workers=2, num_test_workers=2, shuffle_train=True,
...
def get_label_corrupt_data_loader(dataset, noise, norm_stat=None, train_bs=64,
                                  test_bs=64, validation=False,
                                  validation_fraction=0.1, root_dir='data/',
                                  test_only=False, train_only=False, augment=True,
                                  num_train_workers=2, num_test_workers=2,
                                  shuffle_train=True, clean_noisy_split=False, drop_last_train=True):
    if dataset == 'cifar10':
        folder_name = "CIFAR-10-LABELNOISE"
    elif dataset == 'cifar100':
        folder_name = "CIFAR-100-LABELNOISE"
    train_data_cls = partial(LabelCorruptDataset, root=os.path.join(
        root_dir, folder_name, str(noise), 'data.pt')
    )
    test_data_cls = partial(
        LabelCorruptDataset, root=os.path.join(
            root_dir, folder_name, 'val', 'data.pt'
        )
    )
    augment_transform = [
        torchvision.transforms.RandomCrop(32, padding=4),
    ...
    if train_only:
        train_loader = DataLoader(
            train_data, batch_size=train_bs, pin_memory=True,
            shuffle=shuffle_train, drop_last=False,
            num_workers=num_train_workers
        )
        return train_loader
    test_data = test_data_cls(transform=transform)
    test_loader = DataLoader(
        test_data, batch_size=test_bs, pin_memory=True,
        shuffle=False, num_workers=num_test_workers
    )
    if test_only:
        return test_loader
    if validation:
